...
- An uncaught error prints the same `error/2` term that `catch/3` would
  have received, so the message is also the catcher to write:
  `existence_error(procedure, foo/0)`, `type_error(callable, 0.0)`.
