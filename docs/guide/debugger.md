...
files, a call stack of your own predicates, the variables of each frame, stepping
through the four Prolog ports, and moving execution back to a goal that already ran.
When your program calls out to C# or to native C, those frames appear in the *same*
stack, above the Prolog frames that called them.
...
Why the script rather than double-clicking the `.vsix`: VSIXInstaller identifies an
extension by *version*, not by contents. Rebuild without bumping `Version` in
`source.extension.vsixmanifest` and it reports "this extension is already installed to
all applicable products" and keeps the copy you have, having no way to tell two builds
of one version apart. That is correct behaviour for a released extension, where every
iteration carries a new version; it is the wrong shape for the edit-build-install loop,
which is what the script's uninstall-then-install is for.
...
## Moving the program: Set Next Statement

Right-click a goal and pick **Set Next Statement** (Ctrl+Shift+F10), or drag the yellow
arrow onto it. Execution resumes from *that* goal instead of the one it stopped at.

**Forward** simply moves: the goals you skipped over never run. **Backward** is the
interesting direction. A debug session trails everything and records a mark (trail, heap
and choice-point tops) at every debuggable port, so moving back to an earlier goal unwinds
the trail to that goal's mark: every binding made since is undone, and the variables in
Locals go back to what they were. Nothing is replayed and nothing is reconstructed; the
machine is genuinely returned to that point.

Three moves beyond the obvious one:

- **Onto the clause head**: restarts the body. Head unification happened *before* the first
  goal's mark, so the head's own bindings survive: the arguments you were called with stay
  bound.
- **Onto a sibling clause's head**: re-enters the call at that clause. The clauses after it
  remain as alternatives, exactly as if Prolog had chosen it, so failing there falls through
  to the next one, and a cut discards them.
- **Onto a goal in a caller**: select that frame in the Call Stack first, then Set Next
  Statement. The frames above it pop.

**What does not come back**: the database and the outside world. `assertz`, `retract` and
friends are deliberately not trailed (extra backtracking re-running them would be a
correctness bug in its own right), and neither is a line you wrote to a file. Rewinding
past them undoes the bindings, not their effects. That is the one thing to hold in mind
before rewinding over code that asserts.

Refusals are explicit rather than silent, and each says why:

| It says | Because |
|---|---|
| not available at a redo/fail stop | the machine is mid-backtrack; there is no forward position to move to |
| needs last-call optimisation off | with LCO on the frame you are aiming at may no longer exist. `set_prolog_flag(debug_lco, off)`, which is the default under `--debug` |
| line N is not a statement of this clause | only the goal positions the engine publishes with each stop are targets, and Visual Studio checks against that list before it offers the move |
| cannot rewind into this frame at line N | the mark that move needs is gone (a cut discarded it, or the frame is not the one it looks like) |
