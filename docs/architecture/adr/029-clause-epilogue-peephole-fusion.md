...
**Status:** Proposed. Tier-0 interpreter dispatch reduction; Tier-1 unaffected
(the IL describer un-fuses the new opcodes back to the same IL, so promotion is
preserved and codegen is byte-for-byte the pre-fusion shape). Extends chunk 220's
`AllocateGetLevel` / `DeallocateProceed` fusion with the clause-epilogue pairs a
corpus census showed remain.
