...
| `save` | Snapshots the current user dynamic database (all dynamic predicates' clauses) in memory, replacing any previous save/0 snapshot. Engine-internal ($-prefixed) dynamics are excluded. Restore with restore/0. Arity-Prolog compatible builtin. |
