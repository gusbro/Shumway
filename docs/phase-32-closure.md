...
| 2 | **Blittable native form** | `NativeReftype` — `Materialize(Term, Encoding?)→IntPtr` builds the real 32-byte Arity `t_reftype` graph (`AllocHGlobal`; cint 32-bit; `pars` = `t_reftype*` array), `Dematerialize`, and `Free` (Arity `freepar`). Layout fixed **identical to Arity**: `int64 ntype; int64 nelem; t_reftype** pars; union crep`. |
