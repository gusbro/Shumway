...
| 7 | (corpus arc) | **`:- visible` = exported-mutable, not static** | chunk-265 had mis-aliased `:- visible` to `:- dynamic`-as-static, peeling clause-bearing visible predicates out of WAM/IL (real GX ProcDotNet sources: `--dump-wam` showed 0 predicates). Now visible is dynamic (Arity's modifiable "visible table"), so its clauses run as a primed/bakeable snapshot — compiled AND mutable |
...
- Four Arity corpora compile/link/run: `test` 245, `testGen` 311, `testProcDotNet`
  31, plus `prlg_ifce.pl` and the GX ProcDotNet interop sources.
