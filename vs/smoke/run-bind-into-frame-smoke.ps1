...
# DEBUG-config CLIs: the linked exe embeds the engine DLLs sitting next to shumway-link,
# and a func-eval into an OPTIMIZED (Release) engine dies with "Cannot evaluate expression
# because a thread is stopped at a point where garbage collection is impossible". The
# binding evaluation NEEDS the func-eval, so the debuggee must carry unoptimized bits —
# which is also what a user debugging for real builds.
