...
| Delayed choice point — Tier-1 CP-free guard commit SHIPPED default ON, tiers A (inline cmp, 2.6×) + B (binding guards, snapshot/restore, ~1.8×) + G (guard CALLS via forced leaf inlining + det-builtin/is guards, ~2×, boyer −28%); lazy CP materialisation under pending wakeups (entry marks incl. entryE). The clause→ITE AST fold was proven a no-op. Non-inlinable guard callees (fail-continuation + ADR-030 det plumbing) / indexed buckets deferred | ADR-031 |
