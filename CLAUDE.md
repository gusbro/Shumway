...
- ✓ CLP(R) — constraints over the reals. Chunk 99 delivers the linear-equality
  ...
  constraint emitted once, by the variable that owns it). (Historical note,
  now corrected: an early version could not share CLP(R) and CLP(FD) on one
  engine. They have coexisted via the multifile `verify_attributes/4` hook
  since; ADR-040 further makes the hook per attribute module so even two
  export-qualified dialect libraries with module-local hooks coexist.)
...
**Phase 39 — the .NET Framework 4.8 target + the debugger round** — ✅ **Complete** (tagged `phase-39`; closure summary in [`docs/history/phase-39-closure.md`](docs/history/phase-39-closure.md); design in [ADR-043](docs/architecture/adr/043-net-framework-target.md); user guide in [`docs/guide/net-framework-hosts.md`](docs/guide/net-framework-hosts.md)). 32 commits on branch `netfx-target` (kept on origin), squash-merged as `0aa6729`. (1) **OPT-IN net48 multi-target** (`-p:ShumwayNetFx=true`; default builds stay net10-only) across engine + toolchain + REPL + five test suites; ONE polyfill carrier (Core) with net48-only IVT (two visible copies = CS0121/CS8356); C#14 static extension members carry the BCL gaps; only true runtime gaps: DIM and IReadOnlySet→ISet. (2) **Tier-1 runs unchanged on Framework's JIT (x86 AND x64)**; persisted IL emitted NATIVELY on net48 (`AssemblyBuilder` Save mode — 143 tests green with zero edits); deployment matrix: net48-linked bundles (mscorlib refs) bind on BOTH runtimes, net10-linked IL falls back to bytecode on net48 with a warning; `--exe`/`--dll` from the net48 toolchain emit Framework FOLDER apps (`--dll` ships a computed app.config sample). (3) **32-bit facts measured**: memory knees (x86 3–4M list elements, LAA 5–10M, x64 needs `gcAllowVeryLargeObjects`); `t_reftype` offsets hold on x86 (union aligns at 8); real-DLL interop 116/116 per bitness (those tests had self-skipped in every prior gate — CI arms MSVC per lane). (4) **CI**: 3 Windows lanes; traps: vcvars' `Platform` env stamps every assembly (an x86 analyzer can't load into x64 Roslyn — neutralized), per-bucket logs die with the runner (uploaded on failure). (5) **Debugger round (VSIX 0.30)**: Unified Settings hybrid (`VSSDK+VisualStudio.Extensibility`; schema token patched to VS 18's `path`/`pathKind`; values read from the settings.json store — the service isn't proffered to VSSDK packages); launch picks the managed engine by the target exe's runtime; command on Solution Explorer + doc tab; Immediate shows residuals + per-solution bindings, and `!goal` runs ON the suspended frame with the trail as transaction (`!(G, fail)` = dry run). (6) **Engine fixes surfaced**: transplant-source clobber by the nested residual capture (native-clpfd posts exploded); CallIl/ExecuteIl stale per-query IL-snapshot fallback (mid-stop promotions); `consult/1` retries extensionless as `.pl`. Gate: Core 444 / Interpreter 105 / Compiler 364 / ISO 298 / Embedding 3851 (full, Slow incl.) / DialectInterop 9; CI green on the three lanes.

**Phase 38 — WebShumway: the engine in a browser** — ✅ **Complete** (tagged `phase-38`; closure summary in [`docs/history/phase-38-closure.md`](docs/history/phase-38-closure.md); design in [ADR-042](docs/architecture/adr/042-webshumway.md); user guide in [`docs/guide/webshumway.md`](docs/guide/webshumway.md)). 25 commits, +7668/−506. `src/Shumway.Web/` — a **static site** running the full engine on `browser-wasm` with no backend: a Prolog top level, an editor, workspaces, imported libraries, sharing and offline. (1) **Tier-0 only** — a browser forbids runtime codegen, so `RuntimeCaps` (a trimmer `[FeatureSwitchDefinition]`, replacing the wrong `IsDynamicCodeSupported` gate) folds Tier-1 and Sigil out of the payload; programs behave identically, only speed differs. (2) **Threads** (`WasmEnableThreads`) because the search blocks its thread: the runtime moves off the UI thread and the search onto a pool thread — so every `[JSExport]` returns a `Task`, output is POSTED back (JS interop is thread-affine), engine work is serialized behind one gate, and the page must be cross-origin isolated (COOP/COEP `credentialless`) — served by the host where it can, SYNTHESIZED BY THE SERVICE WORKER where it cannot (GitHub Pages), at the cost of one reload on a first visit. (3) **`src/Shumway.TopLevel/`** extracted from the REPL — `TopLevelSession`, `SolutionFormatter`, `SyntaxHighlighter` — so both top levels format an answer the same way. (4) **The editor holds ONE copy of the text** (`contenteditable` with the coloured spans in it): the two-copy overlay only works while both lay out identically, and when they drift the caret lands where the text is not. (5) **MEMFS mirrored to OPFS** — `System.IO` works unmodified, so `consult/1`/`open/4`/the library search path needed no browser casing. (6) **Libraries**: import a COLLECTION (Scryer's `lib/` is one folder and 60 libraries) from a folder or a GitHub URL, dialect-tagged (ADR-040), compiled per library in the background — prioritising what the workspace imports, pausing while you consult, and stopping by itself above 80. Compiled beats source by LAYOUT (bundle at the root, sources in `src/`), not by a read-only flag. Measured ~6× (clpz: 8195/9228/7079 ms from source vs 1266/980/1706 ms from a bundle). (7) **Engine defects the browser surfaced, none browser-specific**: `Console.In` throws on wasm; `PrologEngine.In` and `PrologEngine.Warnings` did not exist; `CollectAttvars` recursed per list element AND conflated attvar/compound addresses (so `Qs ins 1..N` projected no domains); `use_module` of a **bundle** left exports unresolvable; `listing/0` printed library internals; `use_module` never reloaded a changed dependency; new `answer_max_depth` flag and `statistics/0`. Gate: Core 444 / Interpreter 105 / Compiler 364 / ISO 298 / Embedding 3840 / DialectInterop 9.

**Phase 37 — documentation truth, licensing, toolchain & the interop arc** — ✅ **Complete** (tagged `phase-37`; closure summary in [`docs/history/phase-37-closure.md`](docs/history/phase-37-closure.md)). 43 commits, +3976/−6083 — the first phase with a net-deletion diff. (1) **Docs structure**: `docs/` reorganized into `guide/` + `history/` with indexes (plus a root README); [`architecture/invariants.md`](docs/architecture/invariants.md) consolidates the invariant catalog (CLAUDE.md now references it) and [`decision-policy.md`](docs/architecture/decision-policy.md) becomes its own document; generated `predicates.md` moves with the guides. (2) **Truth pass**: every ADR `Status:` line and design doc audited against the code — stale symbols (`Engine`→`Activation`, drifted opcode ids), deferred-but-shipped scope, owner-voice prose, and fictional surfaces (an inline-caching design that was never built) fixed or bannered; the ADR-vs-reference-doc standard codified in `invariants.md`. (3) **Licensing**: MIT + third-party notices + contributing guide; the Logtalk adapter **rewritten from scratch as Shumway's own code**, equivalence proven by a 240-suite fresh-compile sweep; new `version_data` flag. (4) **Toolchain**: `shumway-compile --consult` incremental with self-contained per-module objects, `shumway-link --consult`, `-L dialect:path` on both, `statistics/2`. (5) **Two bundle correctness bugs**, same root cause twice — meta-helper id collisions between the offline per-module numbering and the runtime counter — which broke *every* non-singleton clpz domain narrowing in a compiled bundle and left `-i` snapshots dangling; fixed in `LoadBundle` (`ObserveBundleHelperId`) and `CompileEntryToIl`. They pin the invariant: **tier changes efficiency, never behaviour**. (6) **Interop arc**: `PrologEngine.SolveOnce` (re-entrant host→Prolog solve, ~630 ns vs `QueryAll`'s ~59 µs setup) plus the first measurement against GNU Prolog embedded via P/Invoke — the convenience API loses 3–34× on composites (Term-AST intermediate), the **zero-copy** raw-`Activation` path wins **3–180×** and traversal is essentially free; documented in [`guide/interop.md`](docs/guide/interop.md) + [`benchmarks/cross-engine-comparison.md`](docs/benchmarks/cross-engine-comparison.md), pinned by `ZeroCopyInteropTests`. Gate: Core 444 / Interpreter 105 / Compiler 364 / ISO 298 / Embedding 3752 (full, `Slow` included) / DialectInterop 9.
...
Driven by the `testProc` reference corpus (`C:\temp\testProc\*.c`, Prolog under a
product `#ifdef`, C under `#else`): the uniform pattern is `fill_par(Term,&parNref)`
...
- `t_reftype` layout is **Shumway's own contract** (`int64 ntype; int64 nelem;
  t_reftype** pars; union crep`), blittable; ntype 3 (atom) and 4 (string) both →
  atom on dematerialize. (Corrected 2026-08: an earlier note said "identical to
  Arity", but the Arity material we hold does not declare the struct at all, and
  the reference corpus's `:- c` prototypes treat `reftype` as opaque — the layout is
  Shumway's; native C interops by RECOMPILING against our declaration. Same
  offsets hold on x86: MSVC aligns int64/double at 8 → pars is a 4-byte pointer
  at +16, crep stays at +24, sizeof 32.)
...
- **Blittable native-memory form (done).** `Shumway.Embedding.NativeReftype` —
  ...
  oriented). 14 tests: round-trips, the declared field-offset layout, native-
...
| Redundant-cut elimination via a determinism fixpoint — intra-module elision SHIPPED (default ON; `DeterminismAnalysis` drops a det-prefix last-clause trailing cut → clean tail call); first-arg indexing det is mode-dependent → excluded; linker whole-program closure SHIPPED (ShmoLinker.WholeProgramCutElision) | ADR-030 |
...
| Library loading (`use_module(library(X))`) + scoped module qualification — ACCEPTED (Phase 36): a configurable library search path (`file_search_path(library,Dir)` + `library_directory(Dir)` + C# API + `SHUMWAY_LIBRARY_PATH` + shipped `lib/`) resolves `library(X)` to a `.pl`/`.shum` file. `:- module(Name,[Exports])` (2-arg) is the SOLE trigger for scoped qualification: all its predicates mangle `Name$x` (nothing bare-global), exports are the importable surface, and a per-module import table (`p→Source$p`, built from `use_module/1` all-exports or `use_module/2` selected) resolves calls local→imports→bare-global, at compile time (ModuleRewrite) and runtime (`$mqual`). Two `:- module/2` modules can export the same name (coexist as `A$foo`/`B$foo`). Legacy `:- public`/1-arg-module/prelude/baked-C# libs stay bare-global, unchanged. Linker resolves a `use_module(library(X))` dep C-linker-style: explicit `.shmo` → passed `.shum` archive member → search-path source. Baked-C# libs (clpfd/clpr/coroutining) go through `shumway-compile --consult` (compiles the library's Prolog side into the link; verified to `--exe`); the plain path's `ShmoLibraryDep{Baked}` is recorded but not consumed (parse fails for operator libs, link fails otherwise). `M:goal` syntax still deferred | ADR-038 |
