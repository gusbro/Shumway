...
**Phase 36 — Third-party library loading (`use_module(library(X))`) + scoped module qualification (ADR-038)** — implementation complete, **pending tag** (closure summary in [`docs/phase-36-closure.md`](docs/phase-36-closure.md)). A configurable place for third-party Prolog `.pl` libraries loaded with `use_module(library(X))`, SICStus/Scryer/SWI-compatible. Four components: **(1)** per-engine library search path (`file_search_path(library,Dir)` + `library_directory(Dir)` dynamic facts + `AddLibraryDirectory` API + `SHUMWAY_LIBRARY_PATH` + shipped `lib/`); `use_module(library(X))` resolves `X.pl`/`X.shum` (baked C# → file → CompatLibraries → error); `absolute_file_name(library(X),Abs)`. **(2)** `:- module(Name,[Exports])` (2-arg) is the SOLE trigger for scoped qualification — every predicate mangles `Name$x` (nothing bare-global), exports are the importable surface, a per-module import table resolves calls local→imports→bare-global at compile time (`ModuleRewrite`) AND runtime (`$mqual`, interpreter + IL, via `Activation.CurrentImportMap`); two `:- module/2` modules can export the same name; import-of-non-export is an error. Behaviour change: a 2-arg module's exports are no longer bare-global (2 CompatLibraries tests updated; no repo `.pl` used the form). **(3)** Separate compilation: `shumway-compile` handles `:- module/2` + both `use_module` forms (2-arg filter directly, 1-arg import-all reading `X`'s exports off a compile-time `--library-dir`); `.shmo`/`.shum` carry export-qualification + import table (shared `WriteExportQualification` across both `.shum` writers); `shumway-link` reaches imports (as reachability roots — meta-called imports survive DCE) and C-linker-pulls a `use_module(library(X))` dep from `--library-dir` when unprovided; `LoadBundle` reconstructs manifests cross-process. **(4)** Shipped `lib/lists_ext.pl` on the default path; goal-form `use_module/1` builtin loads + imports into `user` so REPL queries resolve. Deferred: baked-C# libs (clpfd) through separate compilation (parse-time operators + load replay); `M:goal` syntax. Gate: Core 444 / Interpreter 105 / Compiler 360 / ISO 298 / Embedding 3449.
