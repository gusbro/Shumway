...
/// factory is called with an <see cref="Engine"/> whose
/// <see cref="Engine.CurrentBuiltinName"/> is set — i.e. from inside a
...
public static class IsoError
{
    ...
    public static Term TypeError(string expectedType, Term actualValue, Engine? engine = null) =>
    ...
    public static Term InstantiationError(Engine? engine = null) =>
    ...
    public static Term ExistenceError(string kind, Term what, Engine? engine = null) =>
        Wrap(new CompoundTerm(
            "existence_error",
    ...
    public static Term EvaluationError(string kind, Engine? engine = null) =>
        Wrap(new CompoundTerm(
            "evaluation_error",
    ...
    public static Term DomainError(string domain, Term value, Engine? engine = null) =>
        Wrap(new CompoundTerm(
            "domain_error",
    ...
    public static Term PermissionError(string operation, string objectType, Term obj, Engine? engine = null) =>
        Wrap(new CompoundTerm(
            "permission_error",
    ...
    public static Term RepresentationError(string flag, Engine? engine = null) =>
        Wrap(new CompoundTerm(
    ...
    public static Term SyntaxError(string detail, Engine? engine = null) =>
        Wrap(new CompoundTerm(
            "syntax_error",
    ...
    public static Term ResourceError(string resource, Engine? engine = null) =>
        Wrap(new CompoundTerm(
            "resource_error",
    ...
    /// <see cref="SystemError(string, Engine)"/>, which emits
    /// <c>system_error(Detail)</c>.</summary>
    public static Term SystemError(Engine? engine = null) =>
    ...
    /// it.</summary>
    public static Term SystemError(string detail, Engine? engine = null) =>
        Wrap(new CompoundTerm(
            "system_error",
            new Term[] { new AtomTerm(detail) }),
            engine);

    private static Term Wrap(Term kind, Engine? engine) =>
    ...
    /// expect.</summary>
    private static Term BuildContext(Engine? engine)
    {
