...
public sealed partial class BytecodeInterpreter
{
    ...
    // The core (frame, batch, driver entry, WakeReturn) lives on the
    // Activation so Tier-1 region boundaries can fire the same interrupt;
    // this side keeps the interpreter's own concerns: the arity-by-address
    // map for Call/Execute operands, and the drain fallback for a query
    // with no linked driver.

    /// <summary>Alias of <see cref="Activation.WakeReturnCp"/> for the
    /// dispatch loop's CP-jump sites.</summary>
    public const int WakeReturnCp = Shumway.Core.Activation.WakeReturnCp;
    ...
    private int WakeBoundary(ProgramView code, int arity, int resumePc)
    {
        ...
        if (_engine.TryWakeInterrupt(arity, resumePc)) return WakeEntered;
        if (!_engine.HasPendingWakeups) return WakeClear;   // hookless: cleared
        return FlushPendingWakeupsSlow(code) ? WakeClear : WakeFailed;
    }

    private void WakeReturn() => _engine.WakeReturn();
