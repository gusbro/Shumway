...
% Opt-in: PrologEngine.UseClpfd() or use_module(library(clpfd)) loads the
% bundle baked from this file at build time.
%
% Ordinary Prolog over attributed variables. An FD variable carries a clpfd
% attribute fd(Domain, Propagators): the domain is a sorted list of disjoint
% L-H intervals (endpoints integers or inf/sup), the propagators are the
% suspended goals watching it. Posting a constraint suspends its propagators
% on the variables involved and runs them once; narrowing a domain re-runs
