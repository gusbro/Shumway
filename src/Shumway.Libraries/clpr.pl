% CLP(R): constraint logic programming over the reals.
% Opt-in: PrologEngine.UseClpr() or use_module(library(clpr)) loads the
% bundle baked from this file at build time.
%
% Constraints are posted with the {Constraint, ...} wrapper. Equalities feed
% a Gaussian-elimination solver; inequalities are checked for joint
% satisfiability by a simplex on every post, the same one inf/2 and bb_inf/3
% optimise with, so strictness travels with the rows. A disequality is kept
% and fails only once the inequalities pin its linear form to zero. A
% non-linear constraint (product or quotient of two non-constants) is delayed
